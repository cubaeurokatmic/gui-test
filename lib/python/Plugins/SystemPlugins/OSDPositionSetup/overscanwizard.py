# Embedded file name: /usr/lib/enigma2/python/Plugins/SystemPlugins/OSDPositionSetup/overscanwizard.py
...
class OverscanWizard(Screen, ConfigListScreen):
    ...
    def setScreen(self):
        ...
        if self.step == 1:
            ...
            self['introduction'].setText(_("It seems you did not see all the eight arrowheads. This means your TV is has overscan enabled, and is not configured properly.\n\nPlease refer to your TVs manual to find how you can disable overscan on your TV. Look for terms like 'Just fit', 'Full width', etc. If you can't find it, ask other users at forum https://www.openesi.eu.\n\n"))
            self.list.append(getConfigListEntry(_('Did you see all the eight arrowheads?'), self.yes_no))
            self.yes_no.value = True
            self.save_new_position = False
            setPosition(0, 720, 0, 576)
        elif self.step == 3:
            self['introduction'].setText(_('You did not see all eight arrowheads. This means your TV has overscan enabled and presents you with a zoomed-in picture, causing you to loose part of a full HD screen. In addition this you may also miss parts of the user interface, for example volume bars and more.\n\nYou can now try to resize and change the position of the user interface until you see the eight arrowheads.\n\nWhen done press OK.\n\n'))
